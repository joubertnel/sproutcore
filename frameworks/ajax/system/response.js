...
/**
   ...
   cross-domain requests using JSON-P. 
...
SC.JSONPResponse = SC.Response.extend({
  ...
  */
  _didFinish: function(data, status) {
    var error;
    var thisResponse = this;
    status = status || 200;

    if ((status < 200) || (status >= 300)) {
      error = SC.$error('HTTP Request failed', 'Request', status);
      error.set('errorValue', this);
      this.set('isError', YES);
      this.set('errorObject', error);
    }
  ...
  invokeTransport: function() {
    ...
    var statusCodes = [100, 101,
                       400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417,
                       500, 501, 502, 503, 504, 505];
    var statusCodesDispatchTable = {};
    
    statusCodes.forEach(function(httpCode) {
      statusCodesDispatchTable[httpCode] = function(data) { thisResponse._didFinish(data); };      
    });

    $.ajax({ url:JSONPifiedUrl,
             dataType: 'json',
             statusCode: statusCodesDispatchTable,
             success: function(data) { thisResponse._didFinish(data); }
           });
  }
